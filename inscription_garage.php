...
if(isset($_POST['submit'])){
  ...
  if($mailcheckclient > 0 || $mailcheckgarage > 0){ // Si un compte est déjà créé avec cette adresse mail
    header("Location: inscription_garage.php?invalidmail");
    exit;
  }
  else{
    if ($password==$password2){ // Insertion si les mots de passes sont identiques
      $insert_query = pg_query($conn, "INSERT INTO Garage(nomgarage, CRN, adresse, identifiantcompte, motDePassecompte) 
      VALUES ('$nom', '$crn', '$adresse', '$mail', '$password')");
      header("Location: connexion.php");
      exit;
    }else{// Si les mots de passes sont différents
      header("Location: inscription_garage.php?invalidpw");
      exit;
    }
  }
}
...
<meta name="theme-color" content="#563d7c">
  ...
  <body class="text-center">
    <form class="form-signin" method="POST">
        ...
        <h1 class="h3 mb-3 font-weight-normal">Sign up as a garage</h1>
        ...
        <input type="text" id="inputNom" name="inputNom" class="form-control" placeholder="Garage's name" required autofocus>
        <input type="text" id="inputCRN" name="inputCRN" class="form-control" placeholder="CRN" required>
        <input type="text" id="inputAdresse" name="inputAdresse" class="form-control" placeholder="Garage's postal address" required>
        <input type="email" id="inputEmail" name="inputEmail" class="form-control" placeholder="Email addresss" required>
