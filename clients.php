...
switch($tri){
    case 'Tous' : $liste = pg_query($conn, "SELECT idClient, nomClient, prenomClient, mailClient, bloque, nbabandons, points FROM client"); break;
    case 'NonBloqués' : $liste = pg_query($conn, "SELECT idClient, nomClient, prenomClient, mailClient, bloque, nbabandons, points FROM client WHERE bloque = false"); break;
    case 'Bloqués' : $liste = pg_query($conn, "SELECT idClient, nomClient, prenomClient, mailClient, bloque, nbabandons, points FROM client WHERE bloque = true"); break;
    default : $liste = pg_query($conn, "SELECT idClient, nomClient, prenomClient, mailClient, bloque, nbabandons, points FROM client"); break;
}
...
if(isset($_GET['id'])){
    ...
    if(isset($_GET['unblock'])){
        pg_query($conn, "UPDATE client set bloque=false, nbabandons=0 WHERE idclient='$_GET[id]'");
        header("Location:clients");
        exit;
    }
}
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th scope="col">Nombre d'abandons</th>
