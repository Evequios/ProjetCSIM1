...
include('db.php');
if($type == 'garage'){
  $liste = pg_query($conn, "SELECT * FROM voiture 
  INNER JOIN offre ON offre.voiture = voiture.idvoiture
  INNER JOIN garage ON garage.idgarage = offre.garage
  WHERE garage.idgarage = '$id'
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= $donnees['typetransmission']; ?></td>
                    <td><?= $donnees['typecarburant']; ?></td>
                    <td><?= $donnees['kilometrage']; ?></td>
                    <td><?= $donnees['taxe']; ?></td>
                    <td><?= $donnees['cylindree']; ?></td>
                    <td><?php if(pg_num_rows(pg_query($conn, "SELECT * FROM offre WHERE voiture= '$donnees[idvoiture]' AND etatoffre = 'Added'")) > 0){
                        echo 'Yes';}
                        else{
                          echo 'No';
                        }?>
                    <td>
                      <a href="affiche_voiture.php?id=<?=$donnees['idvoiture']?>">View details</a>
                    </td>
                </tr>
                <?php } ?>
            </tbody>
